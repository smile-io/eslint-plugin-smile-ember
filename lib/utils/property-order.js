...
const safeGet = require('deep-value');

const componentClassDefaultProps = [
  'tagName',
  'attributeBindings',
  'classNames',
  'classNameBindings',
...
const mapNameToProp = (props, name) =>
  props.reduce((result, prop) => ({ ...result, [prop]: name }), {});
...
const NAMES = {
  ...
  'public:single-line-function': 'public single-line function',
  'public:multi-line-function': 'public multi-line function',
  'public:empty-method': 'public empty method',

  'private:property': 'private property',
  'private:single-line-function': 'private single-line function',
  'private:multi-line-function': 'private multi-line function',
  'private:empty-method': 'private empty method',

  ...mapNameToProp(
    componentClassDefaultProps,
    "component class' in-built property"
  ),

  'ember-concurrency-task': 'ember concurrency task',
};

function isComponentDefaultProp(node) {
  return componentClassDefaultProps.includes(node.key.name);
}
...
function determinePropertyType(node, parentType) {
  ...
  if (
    ...
    node.value.callee.name === 'task'
  ) {
    return 'ember-concurrency-task';
  }

  if (parentType === 'component') {
    if (ember.isComponentLifecycleHook(node) || isComponentDefaultProp(node)) {
  ...
  if (parentType === 'model') {
    if (safeGet(node, 'value.callee.name') === 'memberAction') {
      return 'member-action';
    } else if (safeGet(node, 'value.callee.name') === 'collectionAction') {
      return 'collection-action';
    } else if (ember.isModule(node.value, 'attr', 'DS')) {
      return 'attribute';
...
function reportUnorderedProperties(node, context, parentType, ORDER) {
  ...
  ember.getModuleProperties(node).forEach((property) => {
    let type = determinePropertyType(property, parentType);

    if (ORDER.includes(`public:${type}`)) {
      let comments = context.getSourceCode().getCommentsBefore(property);
      if (comments.length > 0 && comments[0].value.includes('@public')) {
        type = `public:${type}`;
      } else {
        type = `private:${type}`;
      }
    } else if (ORDER.includes(`private:${type}`)) {
      let comments = context.getSourceCode().getCommentsBefore(property);
    ...
    if (order < maxOrder) {
      ...
      const nextTypeName = getName(
        firstPropertyOfNextType.type,
        firstPropertyOfNextType.node
      );
      ...
      context.report({
        ...
        fix: (fixer) => {
          ...
          const replaceTextRange = [
            foundProperty.start - foundPropertyOffset,
            nextToken.start,
          ];
          const movedProperty = sourceCode.getText(
            property,
            propertyOffset,
            offset
          );
          const restText = sourceCode.getText(
            foundProperty,
            foundPropertyOffset,
            textBetween
          );
          ...
          const movedPropertyWithComma = movedProperty.replace(
            propertyWithEOL,
            replaceWithComma
          );
          ...
          const outputText =
            movedPropertyWithComma + optionalWhitespace + restText;
...
function getCommentOffsetBefore(property, sourceCode) {
  ...
  const isLineComment =
    previousToken.type === 'Line' && commentLineRegExp.test(previousTokenText);
  const isBlockComment =
    previousToken.type === 'Block' &&
    commentBlockRegExp.test(previousTokenText);
